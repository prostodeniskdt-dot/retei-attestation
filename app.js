// SPA: Ryotei, фиксированный порядок ответов, удобный отчёт и печать PDF
...
const STORAGE_KEY = "ryotei_exam_v1";
...
function buildReport(){
  ...
  const timeStr = formatMMSS(elapsedMs);
  const timeFailed = isTimeFailed();
  ...
  state.questions.forEach((q, i) => {
    ...
    detailItems.push(`
      <li class="detail-item">
        <div class="detail-q"><b>${i+1}.</b> ${q.text}</div>
        <div class="detail-a ${ok ? "correct" : "wrong"}">
          Ваш ответ: ${selected ? selected : "<i>не выбран</i>"} ${ok ? "✓" : "✗"}
        </div>
        <div class="detail-r">Верный ответ: <b>${right}</b></div>
      </li>
    `);
  });
  ...
  reportMetaEl.innerHTML = `
    <div class="report-block">
      ...
      <div class="meta-list">
        <div class="meta-row">
          <div class="meta-label">Сотрудник</div>
          <div class="meta-value">${empLine}</div>
        </div>
        <div class="meta-row">
        ...
        <div class="meta-row">
          <div class="meta-label">Статус времени</div>
          <div class="meta-value">${timeFailed ? "<span class='detail-a wrong'>Провалено (превышено 30 минут)</span>" : "Уложился(лась) в 30 минут"}</div>
        </div>
      </div>
    </div>
  `;
    ...
    <div class="report-block">
      ...
      <div class="summary-box">
        <div class="summary-score">
          <div class="summary-score__value">${pct}%</div>
          <div class="summary-score__label">успех</div>
        </div>
        <div class="summary-text">
          Правильных ответов: <b>${correct}</b> из <b>${total}</b>.<br>
          Неправильных или пропущенных: <b>${total - correct}</b>.
        </div>
      </div>
      <div class="summary-time">
        ${timeFailed
          ? "Итоговый статус: <b>аттестация провалена по времени</b>."
          : "Итоговый статус: по времени аттестация пройдена."}
      </div>
    </div>
  `;

  // Блок «Ответы по вопросам»
...
// Печать / PDF: отдельное окно с отчётом
...
<head>
  ...
  <style>
    ...
    .detail-item{ margin-bottom:10px; padding-bottom:8px; border-bottom:1px solid #ddd; }
    .detail-q{ margin-bottom:4px; }
    .detail-a,.detail-r{ font-size:14px; }
  </style>
</head>
<body>
  ${html}
...
shareBtn.addEventListener("click", async () => {
  ...
  const text = `Аттестация Ryotei\nСотрудник: ${empLine}\nНачал(а): ${started}\nЗакончил(а): ${finished}\nИтог: ${correct}/${total} (${pct}%)\nСтатус времени: ${timeStatus}`;

  try{
    if(navigator.share){
      await navigator.share({ title: "Отчёт — Ryotei", text });
